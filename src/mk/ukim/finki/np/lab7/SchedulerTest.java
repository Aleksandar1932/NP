...
class Scheduler<T> {
    ...
    public void add(Date date, T t) {
        schedulerMap.putIfAbsent(date, t);
    }
    ...
    public T next() {
        return Objects.requireNonNull(schedulerMap.entrySet().stream()
                .filter(es -> es.getKey().after(getCurrentDate()))
                .findFirst().orElse(null)).getValue();
    }

    public T last() {
        return Objects.requireNonNull(schedulerMap.entrySet().stream()
                .filter(es -> es.getKey().before(getCurrentDate()))
                .reduce((first, second) -> second).orElse(null)).getValue();

    }

    public ArrayList<T> getAll(Date begin, Date end) {
        return schedulerMap.entrySet().stream()
                .filter(es -> es.getKey().after(begin) && es.getKey().before(end))
                .map(Map.Entry::getValue).collect(Collectors.toCollection(ArrayList::new));
    }

    public T getFirst() {
        return schedulerMap.values().stream().findFirst().orElse(null);
    }

    public T getLast() {
        return schedulerMap.values().stream().reduce((first,second)->second).orElse(null);
    }
}
